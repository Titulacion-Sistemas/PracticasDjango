...
def month(request, year, month):
    ...
    return render_to_response("listado.html", dict(entrada_list=entrada, user=request.user, month=mkmonths_lst(), archive=True))
...
def main(request):
    ...
    try:
        entrada = paginador.page(pagina)
    except (InvalidPage, EmptyPage):
        entrada = paginador.page(paginador.num_pages)

    return render_to_response("listado.html", dict(entrada=entrada, usuario=request.user, entrada_list=entrada.object_list, months=mkmonths_lst()))
